  ...
  const quitarDelCarrito = () => {
    ...
    if (productosCarritoStoredData) {
      ...
      if (productosCarritoParsed instanceof Array) {
        const filteredCarrito = productosCarritoParsed.filter(
          (producto) => producto.productoCarritoId !== product.productoCarritoId
        );

        localStorage.setItem(
          "productosCarrito",
          JSON.stringify(filteredCarrito)
        );
      }
    }
    ...
    enqueueSnackbar("Se quito el producto al carrito!", {
      variant: "warning",
      anchorOrigin: {
        vertical: "top",
        horizontal: "center",
